...
const char* stt(){
   ...
   if(MSP_SUCCESS != ret){
      cout << "ERROR: MSPLogin_failed. Error code: " << ret << endl;
      goto exit;
   }
   else 
   ...
   run_iat (wavfilename, rec_result, session_params);
...
//    char rec_result[BUFFER_SIZE] = {0}; //{NULL};
    ...
    cout << "===>>> start speech to text transfer..." << endl;
    ...
    while (1){
      ...
      if  (MSP_REC_STATUS_SUCCESS == rec_stat){    // alredy has result segment 
         ...
         if (MSP_SUCCESS != errcode){
            cout << "ERROR: QISRGetResult failed. Error code:" << errcode << endl;
            goto iat_exit;
         }
...
//     int cnt_err = 0;
     ...
     while (MSP_REC_STATUS_COMPLETE != rec_stat){
        ...
        if ( MSP_SUCCESS != errcode )
            cout << "Error: QISRGetResult failed. errcode is:" << &errcode << endl;
